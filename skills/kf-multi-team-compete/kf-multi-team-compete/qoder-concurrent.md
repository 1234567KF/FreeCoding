...
```
Agent(subagent_type="kf-hammer-judge",     prompt=<三队产物摘要 + 评分规范 + 任务类型标记>)
Agent(subagent_type="kf-hammer-adversary", prompt=<三队产物摘要 + 8 维度质询框架>)
```

> **裁判 prompt 必须包含任务类型标记**：`taskType={编码开发|原型生成|文档生成|方案评审}`，裁判据此选择维度权重并决定是否输出基准队指定 + 借鉴清单。任务类型从 `hang-state.json` 的 `taskType` 字段读取。

### Step 6 — 主会话汇总（按任务类型分模式）

主会话根据任务类型选择汇总模式：

#### 模式 A: 增量借鉴（编码开发 / 原型生成，分差 ≥ 5%）

```
1. 读取 docs/hammer-judge-report.md
2. 解析基准队（4.1 节）和借鉴清单（4.2 节）
3. 读取基准队完整 Stage 5 产物（{team}-05-final.md）
4. 按借鉴清单优先级处理：
   - 高优先级 → 读取借鉴源文件具体段落（≤50行/次）→ 应用改造
   - 中优先级 → 读取借鉴队 Stage 5 综述段落 → 记录改造
   - 低优先级 → 记录到「待后续评估」清单
5. NEVER 读取借鉴队完整产物文件
6. 生成初版融合方案（增量借鉴模式格式）
```

#### 模式 B: 全量合并回退（编码开发 / 原型生成，分差 < 5%）

```
1. 分差 < 5%，借鉴点高度分散
2. 加载三队完整 Stage 5 产物做智能合并
3. 方案开头标注「全量合并模式（分差<5%回退）」
```

#### 模式 C: 传统融合（文档生成 / 方案评审）

保持原有逻辑：加载三队完整 Stage 5 产物，按分差策略（择优采纳/博采众长/按需融合）合并。
