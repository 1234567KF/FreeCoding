...
**触发时机**：Phase 1.2 歧义检测完成后，锁定规格前

**选择方式**：通过 AskUserQuestion 提供选项：

| 选项 | 深度 | 覆盖步骤 | 说明 |
|------|------|----------|------|
| A | `prototype` | Steps 1-4 | 仅产出原型和PRD草稿，快速验证想法 |
| B | `prd` | Steps 1-10 | 完成需求设计到PRD审定 |
| C | `spec` | Steps 1-15 | 完成技术设计到Spec审定 |
| D | `mvp` | Steps 1-22 | 最小可用产品，含开发和门控 |
| E | `full` | Steps 1-26 | TDD端到端完整交付（默认） |

**执行命令**：
```bash
node {IDE_ROOT}/helpers/hang-state-manager.cjs --init-sop --depth <选择的深度>
```

**规则**：
- 用户未明确选择时默认 `full`
- 深度可随时通过 `depth <level>` 命令调整
- 调整深度不会丢失已完成步骤的状态
...
3. **用户输入命令的处理：**
   ...
   - 用户输入 `nav` / `导航` → 显示 **SOP 导航面板**（见下方「导航面板格式」）
...
```bash
node {IDE_ROOT}/helpers/hang-state-manager.cjs --nav
```

#### 导航面板格式

当用户输入 `nav`、`导航` 或询问“现在在哪/下一步做什么”时，显示导航面板：

```
┌──────────────────────────────────────────────────────────────────┐
│  夯 SOP 导航面板          深度: [full]    当前: Step 3/26        │
├──────────────────────────────────────────────────────────────────┤
│                                                                  │
│  ── 需求设计 ────────────────────────────────────────────  │
│  [v] 1. 需求采集          人类 | PC    → 需求记录文档            │
│  [v] 2. 问题确认          人类 | PC    → 需求确认表格            │
│  [>] 3. 原型制作          AI   | IDE   → PRD                    │
│  [ ] 4. 原型制作          AI   | IDE   → 原型                   │
│  [ ] 5. 需求确认          人类 | PC    → 需求确认记录            │
│  [ ] 6. 需求宣讲          人类 | PC    → 会议问题记录            │
│  [ ] 7. 原型注释追加      AI   | IDE   → 原型注释+补充意见.md   │
│  [ ] 8. 需求问题梳理      人类 | PC    → PRD问题及提案           │
│  [ ] 9. 问题方案汇总澄清  人类 | PC    → PRD审定汇总意见         │
│  [ ] 10. 人机澄清审定需求 人类 | PC    → 审定版PRD               │
│  [ ] 11. 按存量代码生成wiki AI  | IDE   → wiki及原始spec         │
│  [ ] 12. 按PRD产生spec    AI   | IDE   → spec初版               │
│  [ ] 13. 设计问题梳理     人类 | PC    → spec问题及提案           │
│  [ ] 14. 问题方案汇总澄清 人类 | PC    → spec审定汇总意见        │
│  [ ] 15. 生成定版spec     AI   | IDE   → 审定版spec              │
│                                                                  │
│  ── 实现阶段 ────────────────────────────────────────────  │
│  [ ] 16. 测试用例设计     AI   | IDE   → 测试文档               │
│  [ ] 17. 用例评审         人类 | PC    → 测试文档定版            │
│  [ ] 18. 测试脚本开发     AI   | IDE   → 脚本                   │
│  [ ] 19. 功能开发         AI   | IDE   → 代码                   │
│  [ ] 20. codereview       AI   | IDE   → review记录             │
│  [ ] 21. 自动测试执行     AI   | IDE   → 测试报告               │
│  [ ] 22. checklist门控    AI   | IDE   → 门控记录               │
│  [ ] 23. 产生使用手册     AI   | IDE   → 使用手册               │
│  [ ] 24. 人类验收         人类 | PC    → issue/变更              │
│  [ ] 25. 产品定版         人类 | PC    → 定版文档及代码          │
│  [ ] 26. 回写wiki及变动日志 AI | IDE   → wiki更新               │
│                                                                  │
│  ──────────────────────────────────────────────────────────  │
│  下一步: AI 执行「原型制作」→ 产出「PRD」                        │
│  操作: [next] 继续 | [skip] 跳过 | [depth X] 调整深度           │
└──────────────────────────────────────────────────────────────────┘
```
...
**导航规则**：
- 用户可随时输入 `nav` 查看导航面板，不阻塞当前执行
- 用户输入 `status` 时，同时显示看板 + 导航面板
- 中断恢复后，首次回复 MUST 显示导航面板（标注“恢复点”）
- 看板状态文件: `.claude-flow/hang-state.json`（复用，不另建 `.kf/state.json`）

#### 步骤切换引导模板

Team Lead 在每次步骤推进时，必须按以下模板输出引导语：

**AI 步骤引导**（当下一步 role=agent）：
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[Step {N}] AI 执行「{操作名}」
预期产出: {产出物}
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
当前深度 [{depth}] 已完成全部步骤（Step 1-{maxStep}）
产出清单: {各步骤产出物列表}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
如需继续深入，输入: depth <更深级别>
如需查看所有产出，输入: nav
```

**步骤完成推进命令**：
```bash
node {IDE_ROOT}/helpers/hang-state-manager.cjs --step-done
```
...
| 命令 | 作用 | 示例 |
|------|------|------|
| `nav` / `导航` | 显示完整 SOP 导航面板 | `nav` |
| `next` | 执行/进入下一步 | `next` |
| `skip` / `skip N` | 跳过当前步骤/跳过第N步 | `skip 6` |
| `done` | 标记当前人类步骤完成 | `done` |
| `back` | 回退到上一步 | `back` |
| `depth <level>` | 切换任务深度 | `depth mvp` |
| `status` | 刷新看板并展示当前进度 | `status` |
| `fast` | 隐藏导航面板继续执行 | `fast` |
...
| 级别 | 范围 | 说明 |
|------|------|------|
| `prototype` | Steps 1-4 | 仅原型+PRD草稿 |
| `prd` | Steps 1-10 | 完成需求审定 |
| `spec` | Steps 1-15 | 完成技术设计 |
| `mvp` | Steps 1-22 | 最小可用产品 |
| `full` | Steps 1-26 | 端到端完整交付 |
...
模型路由（串行切换）：
  - 前端专家阶段 → model: "sonnet"（kimi-k2.6, 代码/前端/执行层面）
  - 后端专家阶段 → model: "sonnet"（kimi-k2.6, 代码/执行层面）
  - 集成测试阶段 → model: "flash"（deepseek-v4-flash, 测试/缓存优化）
  - Code Review 阶段 → model: "minimax-m2.5"（编程审查SOTA, 100TPS极速）
  - 前端设计师阶段 → model: "sonnet"（kimi-k2.6, 前端/多模态）
  - 协调者（本 Skill 自身）→ model: "opus"（deepseek-v4-pro, 规划+评判层面）
  - 对抗者 → model: "opus"（deepseek-v4-pro, 深度质疑）
  - 汇总者 → model: "opus"（deepseek-v4-pro, 融合决策）
...
从 `hang-state.json` 的 `taskType` 字段读取，或从用户原始触发词推断：

| taskType | 触发来源 | 评分维度表 | 需输出基准队？ |
|----------|---------|-----------|-------------|
| `编码开发` | 用户触发 `/夯` 并要求编码实现 / TDD MVP | 代码质量35% + 测试覆盖25% + 实现一致性20% + 可部署性20% | ✅ 是 |
| `原型生成` | 调用了 `kf-ui-prototype-generator` / 用户说"生成原型" | 设计还原度30% + 交互完整性25% + 代码质量20% + 暗门注释完整15% + 创新价值10% | ✅ 是 |
| `文档生成` | 用户要求生成 PRD/Spec/文档 | 业务契合度30% + 技术可行性25% + 可维护性20% + 风险控制15% + 创新价值10% | ❌ 否（沿用现有融合） |
| `方案评审` | 用户要求评审/分析现有方案 | 同上 | ❌ 否（沿用现有融合） |

裁判根据任务类型选择对应的维度权重表进行评分。具体权重定义见 `agents/kf-hammer-judge.md`。
...
> 裁判输出写入 `docs/hammer-judge-report.md` 后 MUST 通过以下检查。任何项缺失则返回补全。

- [ ] 评分卡三队均已评分，每维度有评语和引用证据
- [ ] **基准队已指定**（仅编码开发/原型生成任务；文档生成/方案评审标记"不适用"即通过）
- [ ] **借鉴清单包含具体文件/行号引用**（仅编码开发/原型生成任务；粒度 ≥ 模块/组件级别）
- [ ] **冲突解决建议已覆盖**所有清单中的矛盾场景（无可列出则写明"无冲突"）
- [ ] pass@k 置信度已完成（仅编码开发任务；其他任务类型标记"跳过"即通过）
...
| 条件 | 模式 | 做法 |
|------|------|------|
| 任务类型 = `编码开发` 或 `原型生成`，且冠军分差 ≥ 5% | **增量借鉴模式** | 以基准队完整输出为骨架，按借鉴清单做增量改造 |
| 任务类型 = `编码开发` 或 `原型生成`，且冠军分差 < 5% | **全量合并模式（回退）** | 三队极其接近，加载三队完整方案做智能合并 |
| 任务类型 = `文档生成` 或 `方案评审` | **传统融合模式** | 保持原有择优采纳/博采众长/按需融合策略 |

> **分差计算公式**：分差 = (冠军总分 - 亚军总分) / 冠军总分 × 100%

---

#### 增量借鉴模式（编码开发 / 原型生成，分差 ≥ 5%）

> **核心原则**：汇总者**NEVER**加载借鉴队的完整产物文件。只读取裁判报告中引用的具体行/段落。

##### Step 4.1 — 读取裁判报告

汇总者读取 `docs/hammer-judge-report.md`，从中解析：
- **基准队**（4.1 节）— 获取队名
- **借鉴清单**（4.2 节）— 获取所有需要借鉴的改造点，按优先级排序
- **冲突解决建议**（4.3 节）— 获取矛盾场景的决策指引

##### Step 4.2 — 读取基准队完整产物

汇总者读取基准队的 Stage 5 产物（`{team}-05-final.md`）及关键阶段产物（按需读取）：
- 架构设计：`{team}-01-architecture.md`
- 实现代码：`{team}-02-impl.md`（或实际代码文件）
- 测试报告：`{team}-03-test-report.md`

> 读取基准队产物时使用 lean-ctx 压缩读取，避免上下文浪费。

##### Step 4.3 — 按优先级应用借鉴清单

| 优先级 | 处理方式 | 读取范围 |
|--------|---------|---------|
| **高** | 读取借鉴源文件的具体段落（只读目标行 ±20 行上下文，不读全量） | 裁判报告中引用的 `{team}-{stage}-*.md#LXX-LYY` |
| **中** | 汇总概述性借鉴（只读借鉴队的 Stage 5 综述中相关段落） | `{team}-05-final.md` 中裁判引用的段落 |
| **低** | 记录到终版方案的「待后续评估」清单，不阻塞当前流程 | 不读取，仅作记录 |

> 若借鉴清单指向的行号过期/无效（文件已变更），跳过该借鉴点并在终版方案中标注 `[行号失效]`。
...
### 增量改造记录
| 借鉴来源 | 改造点 | 改造位置 | 优先级 | 状态 |
|---------|--------|---------|--------|------|
| 红队 | {点} | {文件/模块} | 高/中/低 | 已应用 / 已记录 / [行号失效] |
| 蓝队 | {点} | {文件/模块} | 高/中/低 | 已应用 / 已记录 / [行号失效] |
| 绿队 | {点} | {文件/模块} | 高/中/低 | 已应用 / 已记录 / [行号失效] |
...
1. **NEVER 读取借鉴队的完整产物文件**
2. 只读取裁判报告 `4.2 借鉴清单` 中引用的具体行/段落
3. 借鉴队文件读取每次 ≤ 50 行
4. 若借鉴清单中某队条目 > 10 条，只处理前 10 条（按优先级排序），其余标记「待后续评估」

---

#### 全量合并模式（回退，分差 < 5%）

> 触发条件：三队得分极其接近（分差 < 5%），借鉴点高度分散，逐个提取不如全量合并。

做法：加载三队完整 Stage 5 产物，按传统融合策略做智能合并。

融合产出格式同增量借鉴模式，但在方案开头标注：

```
## 初版融合方案 — 全量合并模式（分差<5%回退）

> 三队分差 = {X}%，极其接近。无法明确指定单一基准队，因此加载三队完整方案做智能合并。
```
