...
**加速策略**：
1. 执行层 agent 用 sonnet/flash（kimi-k2.6 / deepseek-v4-flash）— 输出速度比 Pro 快 2-3x
2. Code Review 用 minimax-m2.5 — SWE-Bench 80.2% SOTA，100TPS极速
3. 阶段产物用 lean-ctx 压缩读取（见 SKILL.md "阶段间产物传递"）
4. 搜索/抓取等辅助操作用 `kf-web-search`/`kf-scrapling`/`kf-opencli` 并行执行
5. 不等待用户确认 — recording 模式，阶段间自动推进
6. 避免在 agent 内做大规模全量文件读取 — 用 ctx_read 模式化读取

### Agent 分工

| Agent | 流水线阶段 | 模型 | 说明 |
|-------|-----------|------|------|
| 前端+后端专家 | Stage 0-1（对齐→架构） | sonnet (kimi-k2.6) | 代码/工具调用稳定 |
| QA 测试设计专家 | Stage 0.5（测试设计先行） | flash (deepseek-v4-flash) | 缓存降低成本 |
| 前端+后端专家 | Stage 2（TDD 微循环） | sonnet (kimi-k2.6) | 编码执行 |
| QA 专家 | Stage 3-3.5（测试+覆盖率） | flash (deepseek-v4-flash) | 测试执行 |
| Code Review | Stage 4（代码审查+TDD 审计） | minimax-m2.5 | 编程审查SOTA |
| 前端设计师 | Stage 5 方案汇总 | sonnet (kimi-k2.6) | 前端/多模态 |
| 协调者（本 Skill） | Pre-Stage + Phase 1-6 | opus (deepseek-v4-pro) | 规划+评判 |
| 对抗者 | Phase 5 对抗质疑 | opus (deepseek-v4-pro) | 深度质疑 |
| 汇总者 | Phase 4 分模式融合 + Phase 6 | opus (deepseek-v4-pro) | 增量/全量/传统融合决策 |

## 任务类型调整

| 任务类型 | Agent 配置 | 流水线策略 | 汇总模式 |
|----------|-----------|-----------|---------|
| **编码开发** | 3 agent/队 × 3 队 + 裁判 + 汇总 + 对抗者 + QA 测试设计 = 15 | 完整 7 阶段 + TDD 微循环 + 覆盖率门控 + 对抗质疑 | 增量借鉴（分差≥5%）/ 全量合并回退（分差<5%） |
| **文档生成** | 2 agent/队 × 3 队 + 裁判 + 汇总 + 对抗者 = 9 | 精简 3 阶段（对齐→撰写→审查）+ 对抗质疑 | 传统融合（择优采纳/博采众长/按需融合） |
| **方案评审** | 2 agent/队 × 3 队 + 裁判 + 汇总 + 对抗者 = 9 | 3 阶段（数据调研→分析→论证）+ 对抗质疑 | 传统融合（择优采纳/博采众长/按需融合） |
...
| Phase 4 → Phase 5 | 初版方案含「待对抗者重点审查的疑点」（增量模式还需含增量改造记录表） | 回退融合 |
...
Phase 4 汇总融合（按任务类型分模式）
  ├── 编码开发/原型生成 + 分差 ≥ 5%
  │     └→ 增量借鉴模式：基准队完整输出 + 按借鉴清单增量改造
  ├── 编码开发/原型生成 + 分差 < 5%
  │     └→ 全量合并模式（回退）：加载三队完整方案做智能合并
  └── 文档生成/方案评审
        └→ 传统融合模式：择优采纳/博采众长/按需融合
       │
       ├── 标注「待对抗者重点审查的疑点」
       └── 进入 Phase 5
